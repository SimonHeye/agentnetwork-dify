...
from services.agent_network import AGENT_NETWORK_DSL_KIND, compile_agent_network_to_dify_dsl
    ...
    def import_app(
        ...
        *,
        ...
        try:
            ...
            if data.get("kind") == AGENT_NETWORK_DSL_KIND:
                content = compile_agent_network_to_dify_dsl(data)
                data = yaml.safe_load(content)
                if not isinstance(data, dict):
                    return Import(
                        id=import_id,
                        status=ImportStatus.FAILED,
                        error="Compiled agent-network DSL must be a mapping",
                    )

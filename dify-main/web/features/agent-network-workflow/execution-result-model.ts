...
export type AgentNetworkExecutionResultPresentation
  ...
  | {
    kind: 'collection'
    items: AgentNetworkExecutionResultPresentation[]
  }
...
export function normalizeAgentNetworkExecutionResult(
  ...
  const collection = parseResultCollection(value)
  if (collection)
    return collection
...
function parseResultCollection(
  value: unknown,
): Extract<AgentNetworkExecutionResultPresentation, { kind: 'collection' }> | null {
  if (Array.isArray(value)) {
    const items = value.map(item => normalizeAgentNetworkExecutionResult(item))
    return items.length ? { kind: 'collection', items } : null
  }
  if (!isRecord(value))
    return null

  const items: AgentNetworkExecutionResultPresentation[] = []
  for (const key of ['text', 'message', 'content', 'answer']) {
  ...
  const resource = parseStructuredResource(value)
  if (resource)
    items.push(resource)

  for (const key of ['images', 'files', 'attachments', 'resources']) {
    const resources = value[key]
    if (Array.isArray(resources))
      items.push(...resources.map(item => normalizeAgentNetworkExecutionResult(item)))
  }

  if (items.length > 1)
    return { kind: 'collection', items }
  if (items.length === 1 && !resource)
    return { kind: 'collection', items }
  return null
}
